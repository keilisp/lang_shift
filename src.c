...
void shift_user_timer(void) {
    // Нужно выключать шифт после прохождения определённого времени, потому что
    // пользователь ожидает как будто шифт на самом деле включён
...
bool shift_once_is_enabled(void) { return shift_once_disable_stage == 2; }
...
uint8_t lang_get_shift_layer_number(void) { return lang_should_be * 2 + 1; }

void lang_synchronize(void) {
    lang_timer = timer_read();
    switch (lang_current_change) {
        case LANG_CHANGE_CAPS: {
            // Костыль, потому что при нажатии Shift+Caps включается режим Caps, а не
            // переключение языка :facepalm:
            if (shift_current == 1) {
                unregister_code(KC_LSHIFT);
        ...
        case LANG_CHANGE_ALT_SHIFT: {
            ...
            // Костыль, потому что при зажатом шифте если хочется нажать клавишу,
            // которая переключает язык, то шифт слетает...
            if (shift_current == 1) {
                register_code(KC_LSHIFT);
            }
        } break;
...
void lang_user_timer(void) {
    // Нужно выключать язык после прохождения определённого времени, потому что
    // пользователь ожидает как будто шифт на самом деле включён
...
bool lang_shift_process_custom_keycodes(Key key, keyrecord_t *record) {
    ...
    // Обрабатываем клавиши, связанные с кастомным шифтом и кастомным
    // переключением языка
    switch (key) {
        case SFT_N_O:
...
bool lang_shift_process_record(Key key, keyrecord_t *record) {
    ...
    // Это нужно отдельно от обработки языка, чтобы шифт мог выключаться для
    // обычных клавиш
